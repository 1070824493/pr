...
class PRAppInfo: PRDynamicCommonParamsProvider {
    ...
    public static var staticCommonParams: [String: Any] = {
        return [
            "os": "ios",
            "osName": PRDeviceUtils.getOSName(),
            "iOSVersion": PRDeviceUtils.getSystemVersion(),
            "deviceType": PRDeviceUtils.getDeviceType().rawValue,
            "vcname": getVCName(),
...
func createWebUrlWithCommonParameters(urlString: String, querys: [String: Any]? = nil) -> URL? {
    ...
    let dynamicCommonParams = PRAppInfo.getWebDynamicCommonParams()
    var parameters = PRAppInfo.staticCommonParams.merging(dynamicCommonParams) { (current, new) in new }
