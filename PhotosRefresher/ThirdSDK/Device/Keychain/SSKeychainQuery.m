...
- (BOOL)deleteItem:(NSError *__autoreleasing *)error {
    ...
    @try {
        ...
        NSLog(@"delete keychain item occur exception: %@", exception);
    } @finally {
...
+ (NSError *)errorWithCode:(OSStatus) code {
	...
	dispatch_once(&onceToken, ^{
		NSURL *url = [[NSBundle bundleForClass:[self class]] URLForResource:@"CommonAI_SSKeychain" withExtension:@"bundle"];
	...
	switch (code) {
        ...
        case errSecTimestampWaiting: {
            message = @"SSKeychainQuery:SecItemCopyMatching_Called_Timeout_5s";
            break;
        }
		default: {
