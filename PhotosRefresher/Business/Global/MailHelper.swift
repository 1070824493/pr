...
final class AppMailHelper: NSObject, MFMailComposeViewControllerDelegate {

    static let shared = AppMailHelper()
    ...
    static func presentSupportEmail() {
        ...
        let appVersion = "\(AppInfo.buildVersion()) (\(AppInfo.getVCName()))"
        let deviceID   = AppInfo.getCuid()
        let deviceInfo = "\(DeviceUtils.getDeviceModel()), \(DeviceUtils.getSystemVersion())"
        ...
        if MFMailComposeViewController.canSendMail() {
            ...
            vc.mailComposeDelegate = AppMailHelper.shared
