...
class PRConfigManager: ObservableObject {
    
    public static let shared = PRConfigManager()
    
    @Published var appConfig: PRAppConfig? = nil
    ...
    func refreshConfig() async -> Bool {
        do {
            let res: PRCommonResponse<PRAppConfig> = try await PRRequestHandlerManager.shared.PRrequest(url: ApiConstants.photosrefresher_init_config, method: .get)
            if !res.succeed() {
                return false
            }
    ...
    private func loadCache(file: URL) -> Bool {
        if let data = try? Data(contentsOf: file) {
            ...
            do {
                let savedData = try decoder.decode(PRAppConfig.self, from: data)
